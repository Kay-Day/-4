...
public class ServerCore {
    ...
    private ObjectOutputStream obtOutputCient;
    private ObjectInputStream obInputStream;
    private boolean isStop = false , isExit = false;
...
//	chờ đợi kết nối từ một client đến server
...
//	 lưu trữ thông tin của một đối tượng Peer mới vào danh sách dataPeer
...
//	 xử lý kết nối mạng đến từ client
				...
				while (!isStop) {
					if(waitForConnection()) {
						if(isExit) {
							...
							obtOutputCient = new ObjectOutputStream(connection.getOutputStream());
							obtOutputCient.writeObject(sendSessionAccept());
							obtOutputCient.flush();
							obtOutputCient.close();
						}
		
					}else {
						obtOutputCient = new ObjectOutputStream(connection.getOutputStream());
						obtOutputCient.writeObject(Tags.SESSION_DENY_TAG);
						obtOutputCient.flush();
						obtOutputCient.close();
					}
